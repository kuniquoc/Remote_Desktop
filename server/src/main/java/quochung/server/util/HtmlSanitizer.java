...
public class HtmlSanitizer {
    ...
    private static final String[] ALLOWED_TAGS = { "b", "i", "em", "strong", "a", "p", "br", "ul", "ol", "li" };
    ...
    public static String escapeHtml(String input) {
        ...
        while (matcher.find()) {
            ...
            escapedHtml.append(escapeSpecialChars(input.substring(lastMatchEnd, matcher.start())));

            // Kiểm tra thẻ có nằm trong danh sách cho phép không
            if (isAllowedTag(tagName)) {
                escapedHtml.append(tag); // Giữ nguyên thẻ an toàn
            } else {
                escapedHtml.append("&lt;").append(tagName).append("&gt;"); // Thay thế thẻ không an toàn
            }
